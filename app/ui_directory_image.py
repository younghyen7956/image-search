...
def initialize():
    ...
    repo.get_model()
    repo.connectDB()
    print("✅ Gradio startup: model loaded and DB connected.")
...
async def image_search_gradio(input_image):
    ...
    images = []
    for r in results:
        filename = r["filename"]  # ex. "08_081_08011004_160273687615048.jpg"
        img_path = IMAGE_DIR / filename
        print("Looking for:", img_path)  # 터미널 디버깅용

        if img_path.exists():
            images.append(Image.open(img_path))
        else:
            # 파일이 없으면 빨간 대체 이미지
            images.append(Image.new("RGB", (224,224), color=(255,0,0)))
    return images

# 4) Gradio 블록 구성
with gr.Blocks() as demo:
    gr.Markdown("## 이미지 유사도 검색")
    with gr.Row():
        inp = gr.Image(type="pil", label="검색할 이미지를 업로드하세요")
        out = gr.Gallery(label="검색 결과", columns=5, height="auto")
    btn = gr.Button("검색 시작")
    btn.click(fn=image_search_gradio, inputs=inp, outputs=out)
